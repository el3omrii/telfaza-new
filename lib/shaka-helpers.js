...
export function buildShakaConfig({ link, drm, clearkeys }) {
  ...
  const config = {
	...
	sequenceMode: true,
      ...
      streaming: {
        // Force Shaka to start playing after it receives just 1 segment (5 seconds)
        // instead of waiting for 3 segments (15 seconds)
        rebufferingGoal: 2, 
        bufferingGoal: 10,
	smallGapLimit: 0.5,
    
        // Automatically manage tiny audio/video drift common in combined .ts files
        safeSeekOffset: 2,
        stallThreshold: 1,
      },
